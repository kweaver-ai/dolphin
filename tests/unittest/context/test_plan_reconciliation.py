...
class TestPlanReconciliation:
    ...
    async def test_reconciliation_after_restore(self):
        ...
        # just having a new PlanToolkit instance is enough)
        snapshot = context.export_runtime_state(frame_id="frame_1")
        
        new_context = Context()
        new_context.apply_runtime_state(snapshot)
        
        # Create a new PlanToolkit
        skillkit = PlanToolkit(context=new_context)
        ...
        # But wait, PlanToolkit uses ExploreBlock which needs LLM etc.
