#!/usr/bin/env python3
"""
Unit tests for ToolMatcher utility class.
...
def create_mock_skill(name: str, owner: Optional[str] = None) -> Mock:
    """Helper to create a mock ToolFunction."""
    skill = Mock()
    skill.get_function_name.return_value = name
    skill.get_owner_toolkit.return_value = owner
    # Add owner_name property that ToolMatcher now uses
...
class TestMatchSkillName(unittest.TestCase):
    ...
    def test_exact_match(self):
        ...
        self.assertTrue(ToolMatcher.match_tool_name("my_tool", "my_tool"))
        self.assertFalse(ToolMatcher.match_tool_name("my_tool", "other_tool"))

    def test_wildcard_suffix(self):
        """Pattern 'prefix*' should match names starting with prefix."""
        self.assertTrue(ToolMatcher.match_tool_name("playwright_navigate", "playwright*"))
        self.assertTrue(ToolMatcher.match_tool_name("playwright", "playwright*"))
        self.assertFalse(ToolMatcher.match_tool_name("other_tool", "playwright*"))

    def test_wildcard_prefix(self):
        """Pattern '*suffix' should match names ending with suffix."""
        self.assertTrue(ToolMatcher.match_tool_name("browser_navigate", "*_navigate"))
        self.assertFalse(ToolMatcher.match_tool_name("browser_click", "*_navigate"))

    def test_wildcard_middle(self):
        """Pattern 'prefix*suffix' should match names with prefix and suffix."""
        self.assertTrue(ToolMatcher.match_tool_name("playwright_browser_navigate", "playwright*navigate"))
        self.assertFalse(ToolMatcher.match_tool_name("playwright_browser_click", "playwright*navigate"))

    def test_single_char_wildcard(self):
        """Pattern with '?' should match single character."""
        self.assertTrue(ToolMatcher.match_tool_name("tool_v1", "tool_v?"))
        self.assertTrue(ToolMatcher.match_tool_name("tool_v2", "tool_v?"))
        self.assertFalse(ToolMatcher.match_tool_name("tool_v10", "tool_v?"))
...
class TestMatchSkillNames(unittest.TestCase):
    ...
    def test_match_any_pattern(self):
        ...
        self.assertTrue(ToolMatcher.match_tool_names("playwright_navigate", patterns))
        self.assertTrue(ToolMatcher.match_tool_names("_python", patterns))
        self.assertTrue(ToolMatcher.match_tool_names("sql_query", patterns))
        self.assertFalse(ToolMatcher.match_tool_names("other_tool", patterns))

    def test_empty_patterns(self):
        """Empty pattern list should not match anything."""
        self.assertFalse(ToolMatcher.match_tool_names("any_tool", []))
...
class TestFilterSkillsByPattern(unittest.TestCase):
    ...
    def test_filter_by_single_pattern(self):
        ...
        result = ToolMatcher.filter_tools_by_pattern(self.skills, "playwright*")
    ...
    def test_filter_by_multiple_patterns(self):
        ...
        result = ToolMatcher.filter_tools_by_patterns(self.skills, ["playwright*", "_python"])
        self.assertEqual(len(result), 3)


class TestGetOwnerToolkits(unittest.TestCase):
...
class TestSplitNamespacedPattern(unittest.TestCase):
    ...
    def test_plain_pattern(self):
        ...
        owner_names = {"resource_toolkit", "mcp_toolkit"}
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern("my_tool", owner_names)
    ...
    def test_namespaced_pattern(self):
        ...
        owner_names = {"resource_toolkit", "mcp_toolkit"}
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern(
            "resource_toolkit.load_*", owner_names
        )
        self.assertEqual(owner, "resource_toolkit")
        self.assertEqual(suffix, "load_*")
        self.assertTrue(is_ns)

    def test_toolkit_name_only(self):
        """Toolkit name alone should expand to '.*'."""
        owner_names = {"resource_toolkit"}
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern(
            "resource_toolkit", owner_names
        )
        self.assertEqual(owner, "resource_toolkit")
    ...
    def test_empty_owner_names(self):
        ...
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern(
            "resource_toolkit.tool", set()
        )
        self.assertIsNone(owner)
        self.assertEqual(suffix, "resource_toolkit.tool")
        self.assertFalse(is_ns)
...
class TestMatchSkill(unittest.TestCase):
    ...
    def test_plain_pattern_match(self):
        ...
        skill = create_mock_skill("my_tool", "some_toolkit")
        self.assertTrue(ToolMatcher.match_tool(skill, "my_tool"))
        self.assertTrue(ToolMatcher.match_tool(skill, "my_*"))
        self.assertFalse(ToolMatcher.match_tool(skill, "other_tool"))

    def test_namespaced_pattern_match(self):
        """Namespaced pattern should match by owner and name."""
        skill = create_mock_skill("load_resource", "resource_toolkit")
        owner_names = {"resource_toolkit"}

        # Should match: correct owner + matching name
        self.assertTrue(ToolMatcher.match_tool(skill, "resource_toolkit.load_*", owner_names))

        # Should not match: wrong owner
        skill_other = create_mock_skill("load_resource", "other_toolkit")
        self.assertFalse(ToolMatcher.match_tool(skill_other, "resource_toolkit.load_*", owner_names))

    def test_toolkit_name_matches_all(self):
        """Toolkit name alone should match all skills from that skillkit."""
        skill = create_mock_skill("any_tool", "resource_toolkit")
        owner_names = {"resource_toolkit"}
        self.assertTrue(ToolMatcher.match_tool(skill, "resource_toolkit", owner_names))
...
class TestMatchSkillsBatch(unittest.TestCase):
    ...
    def setUp(self):
        self.skills = [
            create_mock_skill("playwright_navigate", "mcp_toolkit"),
            create_mock_skill("playwright_click", "mcp_toolkit"),
            create_mock_skill("_python", None),
            create_mock_skill("load_resource", "resource_toolkit"),
        ]
        self.owner_names = {"mcp_toolkit", "resource_toolkit"}

    def test_batch_match_multiple_patterns(self):
        """Should match skills against multiple patterns."""
        patterns = ["playwright*", "_python"]
        matched, any_ns = ToolMatcher.match_tools_batch(
    ...
    def test_batch_match_deduplication(self):
        ...
        matched, _ = ToolMatcher.match_tools_batch(
            self.skills, patterns, self.owner_names
        )
    ...
    def test_batch_match_namespaced_patterns(self):
        ...
        patterns = ["mcp_toolkit.playwright*", "_python"]
        matched, any_ns = ToolMatcher.match_tools_batch(
            self.skills, patterns, self.owner_names
        )
...
class TestMissingMethods(unittest.TestCase):
    ...
    def test_find_first_matching_tool_found(self):
        """Should return first matching skill."""
        result = ToolMatcher.find_first_matching_tool(self.skills, "tool_*")
        self.assertEqual(result.get_function_name(), "tool_a")

    def test_find_first_matching_tool_not_found(self):
        """Should return None when no match."""
        result = ToolMatcher.find_first_matching_tool(self.skills, "nonexistent*")
        self.assertIsNone(result)

    def test_find_first_matching_tool_exact_match(self):
        """Should find skill by exact name."""
        result = ToolMatcher.find_first_matching_tool(self.skills, "tool_b")
        self.assertEqual(result.get_function_name(), "tool_b")

    def test_get_matching_tools_with_none(self):
        """None skill_names should return all skills."""
        result = ToolMatcher.get_matching_tools(self.skills, None)
        self.assertEqual(len(result), 3)

    def test_get_matching_tools_with_patterns(self):
        """Should filter using pattern list."""
        result = ToolMatcher.get_matching_tools(self.skills, ["tool_a", "tool_c"])
        self.assertEqual(len(result), 2)

    def test_get_matching_tools_by_names_exact(self):
        """Should match by exact names only (no wildcards)."""
        result = ToolMatcher.get_matching_tools_by_names(self.skills, ["tool_a", "tool_c"])
        self.assertEqual(len(result), 2)
        names = [s.get_function_name() for s in result]
        self.assertIn("tool_a", names)
        self.assertIn("tool_c", names)

    def test_get_matching_tools_by_names_no_wildcard(self):
        """Wildcards should NOT work in get_matching_tools_by_names."""
        result = ToolMatcher.get_matching_tools_by_names(self.skills, ["tool_*"])
        self.assertEqual(len(result), 0)  # No exact match for "tool_*"

    def test_get_tool_by_name_exact(self):
        """Should return skill by exact name."""
        result = ToolMatcher.get_tool_by_name(self.skills, "tool_b")
        self.assertEqual(result.get_function_name(), "tool_b")

    def test_get_tool_by_name_pattern(self):
        """Should return first skill matching pattern."""
        result = ToolMatcher.get_tool_by_name(self.skills, "*_c")
        self.assertEqual(result.get_function_name(), "tool_c")

    def test_get_tool_by_name_not_found(self):
        """Should return None when not found."""
        result = ToolMatcher.get_tool_by_name(self.skills, "nonexistent")
        self.assertIsNone(result)
...
class TestEdgeCases(unittest.TestCase):
    ...
    def test_case_sensitivity(self):
        ...
        import sys
        self.assertTrue(ToolMatcher.match_tool_name("MyTool", "MyTool"))
        if sys.platform == "win32":
            ...
            self.assertFalse(ToolMatcher.match_tool_name("MyTool", "mytool"))
            self.assertFalse(ToolMatcher.match_tool_name("mytool", "MyTool"))

    def test_case_sensitivity_with_wildcard(self):
        """Case sensitivity should apply to wildcards too (platform-dependent)."""
        import sys
        self.assertTrue(ToolMatcher.match_tool_name("MyTool", "My*"))
        if sys.platform == "win32":
            # On Windows, fnmatch is case-insensitive
            self.assertTrue(ToolMatcher.match_tool_name("MyTool", "my*"))
        else:
            # On Unix-like systems, fnmatch is case-sensitive
            self.assertFalse(ToolMatcher.match_tool_name("MyTool", "my*"))

    def test_special_characters_underscore_prefix(self):
        """Skills with underscore prefix should match correctly."""
        self.assertTrue(ToolMatcher.match_tool_name("_private_tool", "_*"))
        self.assertTrue(ToolMatcher.match_tool_name("_private_tool", "_private_*"))

    def test_special_characters_hyphen(self):
        """Skills with hyphens should match correctly."""
        self.assertTrue(ToolMatcher.match_tool_name("tool-v2", "tool-*"))
        self.assertTrue(ToolMatcher.match_tool_name("my-cool-tool", "*-cool-*"))
    ...
    def test_pattern_with_dot_not_namespace(self):
        ...
        owner_names = {"other_toolkit"}
        # Should match the full pattern as skill name
        self.assertTrue(ToolMatcher.match_tool(skill, "some.tool.name", owner_names))
        self.assertTrue(ToolMatcher.match_tool(skill, "some.tool.*", owner_names))
    ...
    def test_namespaced_pattern_skill_without_owner(self):
        ...
        owner_names = {"resource_toolkit"}
        self.assertFalse(
            ToolMatcher.match_tool(skill, "resource_toolkit.load_*", owner_names)
        )

    def test_empty_pattern_string(self):
        """Empty pattern should match only empty skill name."""
        self.assertTrue(ToolMatcher.match_tool_name("", ""))
        self.assertFalse(ToolMatcher.match_tool_name("tool", ""))
    ...
    def test_filter_empty_skills_list(self):
        ...
        result = ToolMatcher.filter_tools_by_pattern([], "any*")
    ...
    def test_filter_by_patterns_empty_skills_list(self):
        ...
        result = ToolMatcher.filter_tools_by_patterns([], ["any*", "other*"])
        self.assertEqual(result, [])

    def test_batch_match_tool_matches_multiple_patterns(self):
        """Skill matching multiple patterns should appear only once (dedup)."""
        skills = [create_mock_skill("abc_xyz")]
        patterns = ["abc*", "*xyz", "abc_xyz"]
        matched, _ = ToolMatcher.match_tools_batch(skills, patterns)
        self.assertEqual(len(matched), 1)

    def test_match_star_only(self):
        """Single * should match any skill name."""
        self.assertTrue(ToolMatcher.match_tool_name("anything", "*"))
        self.assertTrue(ToolMatcher.match_tool_name("", "*"))
        self.assertTrue(ToolMatcher.match_tool_name("very_long_skill_name_here", "*"))

    def test_double_wildcard(self):
        """Pattern with ** should work like * (fnmatch behavior)."""
        self.assertTrue(ToolMatcher.match_tool_name("abc", "**"))
        self.assertTrue(ToolMatcher.match_tool_name("abc", "a**"))
...
class TestNamespaceComplexCases(unittest.TestCase):
    ...
    def test_owner_name_with_underscore(self):
        ...
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern("mcp.tool", owner_names)
    ...
    def test_trailing_dot_in_pattern(self):
        ...
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern("skillkit.", owner_names)
    ...
    def test_multiple_dots_in_suffix(self):
        ...
        owner_names = {"owner"}
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern("owner.a.b.c", owner_names)
    ...
    def test_owner_name_is_prefix_of_another(self):
        ...
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern("api.tool", owner_names)
        self.assertEqual(owner, "api")
        self.assertEqual(suffix, "tool")

        # "api.v2.tool" should match "api.v2" (longer prefix wins)
        owner, suffix, is_ns = ToolMatcher.split_namespaced_pattern("api.v2.tool", owner_names)
    ...
    def test_namespaced_match_wrong_owner(self):
        ...
        skill = create_mock_skill("load_data", "other_toolkit")
        owner_names = {"resource_toolkit", "other_toolkit"}

        # Pattern specifies resource_toolkit, but skill belongs to other_toolkit
        self.assertFalse(
            ToolMatcher.match_tool(skill, "resource_toolkit.load_*", owner_names)
        )

        # Same pattern should match if skill belongs to correct owner
        skill_correct = create_mock_skill("load_data", "resource_toolkit")
        self.assertTrue(
            ToolMatcher.match_tool(skill_correct, "resource_toolkit.load_*", owner_names)
        )
    ...
    def test_batch_match_mixed_namespaced_and_plain(self):
        ...
        matched, any_ns = ToolMatcher.match_tools_batch(skills, patterns, owner_names)
    ...
    def test_owner_only_pattern_matches_all_skills_from_owner(self):
        """Toolkit name alone should match all skills from that owner."""
        skills = [
            create_mock_skill("skill_1", "my_toolkit"),
            create_mock_skill("skill_2", "my_toolkit"),
            create_mock_skill("skill_3", "other_toolkit"),
        ]
        owner_names = {"my_toolkit", "other_toolkit"}
        ...
        self.assertEqual(len(matched), 2)
        for s in matched:
            self.assertEqual(s.get_owner_toolkit(), "my_toolkit")
