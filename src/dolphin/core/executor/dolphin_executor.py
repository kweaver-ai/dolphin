...
class DolphinExecutor:
    ...
    async def run_coroutine(self, frame_id: str, progress_callback=None):
        ...
        try:
            ...
            while pointer < len(blocks):
                # Check for user interrupt before each block
    ...
    async def resume_coroutine(self, handle, updates=None):
        ...
        user_interrupt_input = None
        if updates:
            if KEY_USER_INTERRUPT_INPUT in updates:
                user_interrupt_input = updates.get(KEY_USER_INTERRUPT_INPUT)
            # Apply updates data
            for key, value in updates.items():
                if key == KEY_USER_INTERRUPT_INPUT:
                    continue
        ...
        if is_intervention_resume:
            ...
            if frame.wait_reason == WaitReason.USER_INTERRUPT:
                ...
                if user_interrupt_input is not None:
                    # Inject into scratchpad to preserve temporal order:
                    # QUERY -> partial ASSISTANT output -> user recovery input
                    self.context.add_user_message(
                        user_interrupt_input,
                        bucket=BuildInBucket.SCRATCHPAD.value,
                    )
                    logger.info(
                        f"UserInterrupt resume: injected user input of length {len(user_interrupt_input)}"
                    )
